...
(function () {
  ...
  function filterNews() {
    const searchTerm =
      document.getElementById("newsSearchInput")?.value.toLowerCase() || "";
    const statusFilter =
      document.getElementById("newsStatusFilter")?.value || "all";
    const sortOrder =
      document.getElementById("newsSortOrder")?.value || "newest";
  ...
  window.openAddNewsModal = function () {
    ...
    document.getElementById("newsPublishedDate").value = new Date()
      .toISOString()
      .split("T")[0];
    ...
    fetch(`api/News.php?action=fetch_single&id=${newsId}`)
      ...
      .then((data) => {
        if (data.success) {
          ...
          window.showToast("Failed to load news details", "error");
        }
      })
      .catch((error) => {
        console.error("Error loading news:", error);
        window.showToast(
          "An error occurred while loading news details",
          "error",
        );
      });
  };
    ...
    fetch(`api/News.php?action=fetch_single&id=${newsId}`)
      ...
      .then((data) => {
        if (data.success) {
          ...
          window.showToast("Failed to load news for editing", "error");
        }
      })
      .catch((error) => {
        console.error("Error loading news:", error);
        window.showToast("An error occurred while loading news", "error");
      });
  };
  ...
  function populateEditForm(news) {
    ...
    document.getElementById("newsPublishedDate").value =
      news.published_date || "";
  ...
  function handleNewsFormSubmit(e) {
    ...
    fetch("api/News.php", {
      ...
      .catch((error) => {
        ...
        window.showToast("An error occurred while saving news", "error");
      });
  }
  ...
  window.deleteNews = async function (newsId) {
    try {
      ...
      window.showToast("An error occurred while deleting news", "error");
    }
  };
