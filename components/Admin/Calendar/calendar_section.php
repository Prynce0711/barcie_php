...
<div class="d-flex align-items-center gap-2 flex-wrap py-1">
  <div class="btn-group btn-group-sm" role="group" aria-label="Filter">
    <button type="button" class="btn btn-sm btn-outline-secondary admin-room-filter-btn active"
      data-filter="all">All</button>
    <button type="button" class="btn btn-sm btn-outline-secondary admin-room-filter-btn"
      data-filter="room">Rooms</button>
    <button type="button" class="btn btn-sm btn-outline-secondary admin-room-filter-btn"
...
<script>
  ...
  if ($items_result && $items_result->num_rows > 0) {
    while ($item = $items_result->fetch_assoc()) {
      ...
      // Only pass image paths that really exist in the workspace to avoid repeated browser 404s.
      $images_raw = '';
      if (!empty($item['images'])) {
        $decoded_images = json_decode($item['images'], true);
        if (!is_array($decoded_images)) {
          $decoded_images = [$item['images']];
        }
        $valid_images = [];
        foreach ($decoded_images as $candidate) {
          $p = '';
          if (is_array($candidate)) {
            $p = (string) ($candidate['url'] ?? $candidate['src'] ?? $candidate['path'] ?? '');
          } else {
            $p = (string) $candidate;
          }
          $p = ltrim(trim($p), '/\\');
          ...
          $abs = $project_root . DIRECTORY_SEPARATOR . str_replace(['/', '\\'], DIRECTORY_SEPARATOR, $p);
          if (file_exists($abs)) {
            $valid_images[] = str_replace('\\', '/', $p);
          }
        }
        if (!empty($valid_images)) {
          $images_raw = addslashes(json_encode($valid_images));
        }
      }

      $image_raw = '';
      if (!empty($item['image'])) {
        $single = ltrim(trim((string) $item['image']), '/\\');
        $single_abs = $project_root . DIRECTORY_SEPARATOR . str_replace(['/', '\\'], DIRECTORY_SEPARATOR, $single);
        if ($single !== '' && file_exists($single_abs)) {
          $image_raw = addslashes(str_replace('\\', '/', $single));
        } else {
          $image_raw = addslashes($default_preview);
        }
      } else {
        $image_raw = addslashes($default_preview);
      }
