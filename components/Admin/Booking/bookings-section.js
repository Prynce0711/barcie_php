...
function resetFilters() {
  ...
  try {
    ...
    if (section) {
      if (typeof window.showTableSpinner === "function") {
        ...
        try {
          removeSectionSpinner = window.showTableSpinner(section);
        } catch (e) {
          removeSectionSpinner = function () {};
        }
      } else {
        ...
        removeSectionSpinner = function () {
          ...
          try {
            if (computed === "static") section.style.position = prevPos || "";
          } catch (e) {}
        };
      }
    }
  } catch (e) {
    console.error("section spinner error", e);
  }
  ...
  tableIds.forEach((id) => {
    try {
      ...
      if (typeof window.showTableSpinner === "function") {
        ...
        // fallback overlay
        const parent =
          tbl.closest && tbl.closest(".table-responsive")
            ? tbl.closest(".table-responsive")
            : tbl;
        ...
        removers.push(function () {
          ...
          try {
            if (computed === "static") parent.style.position = prevPos || "";
          } catch (e) {}
        });
      }
    } catch (e) {
  ...
  try {
    ...
    const allBtn = document.querySelector('.type-filter-btn[data-type=""]');
    if (allBtn) {
  ...
  try {
    if (
      ...
      typeof window._discountsPagination.goToPage === "function"
    )
      window._discountsPagination.goToPage(1);
  } catch (e) {}
  try {
    if (
      window._paymentsPagination &&
      typeof window._paymentsPagination.goToPage === "function"
    )
      window._paymentsPagination.goToPage(1);
  } catch (e) {}
...
function updateBookingStatus(bookingId, newStatus) {
  ...
  showConfirmModal(
    `Are you sure you want to change this booking status to "${newStatus.replace("_", " ")}"?`,
  ).then((confirmed) => {
    if (!confirmed) return;
...
function processDiscount(discountId, action) {
  ...
  showConfirmModal(
    `Are you sure you want to ${actionText} this discount application?`,
  ).then((confirmed) => {
    if (!confirmed) return;
