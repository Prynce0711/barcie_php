          ...
          <form method="POST" action="" enctype="multipart/form-data" class="border-top pt-3">
            ...
            <div class="row">
              ...
              <div class="col-12 mb-3">
                    ...
                    <?php foreach ($addons as $aidx => $a): ?>
                      <div class="card p-2 mb-2 addon-row" data-idx="<?= $aidx ?>">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                          ...
                          <button type="button" class="btn btn-danger btn-sm addon-remove-btn" title="Remove add-on"><i
                              class="fas fa-trash"></i></button>
                        </div>
                        <div class="row g-2">
                          <div class="col-12 mb-2">
                            <input type="text" name="addons[name][]" class="form-control" placeholder="Add-on name"
                          ...
                          <div class="col-6 mb-2">
                            ...
                            ?>
                            <input type="text" name="addons[price][]" class="form-control addon-price-input"
                              placeholder="Price" value="<?= htmlspecialchars($priceVal) ?>">
                          </div>
                          <div class="col-6 mb-2">
                            <select name="addons[type][]" class="form-select">
                              <option value="Per Event" <?= ($a['type'] ?? '') === 'Per Event' ? 'selected' : '' ?>>Per Event
                              </option>
                              <option value="Per Day" <?= ($a['type'] ?? '') === 'Per Day' ? 'selected' : '' ?>>Per Day</option>
                              <option value="Per Night" <?= ($a['type'] ?? '') === 'Per Night' ? 'selected' : '' ?>>Per Night
                              </option>
                            </select>
                          </div>
                        </div>
                    ...
                    <?php foreach ($images as $idx => $imgPath):
                      ...
                      if (file_exists($imageFullPath)) {
                        ...
                        <div class="position-relative image-entry" data-image-path="<?= htmlspecialchars($imgPath) ?>"
                          data-image-index="<?= $idx ?>" id="imageEntry<?= $item['id'] ?>_<?= $idx ?>">
                          <img src="<?= htmlspecialchars($displayImagePath) ?>" alt="Image <?= $idx + 1 ?>"
                            style="width: 80px; height: 80px; object-fit: cover;" class="rounded preview-image"
                            id="previewImg<?= $item['id'] ?>_<?= $idx ?>">

                          <!-- Hidden replace input for this image -->
                          <input type="file" name="replace_images[<?= $item['id'] ?>][<?= $idx ?>]" accept="image/*"
                            class="d-none replace-input" id="replaceInput<?= $item['id'] ?>_<?= $idx ?>">

                          <div class="btn-group position-absolute top-0 end-0 m-1">
                            <button type="button" class="btn btn-danger btn-sm p-1" style="font-size: 10px;"
                              onclick="removeImage<?= $item['id'] ?>('<?= htmlspecialchars($imgPath, ENT_QUOTES) ?>')"
                              title="Remove image">
                              <i class="fas fa-times"></i>
                            </button>
                            <button type="button" class="btn btn-secondary btn-sm p-1" style="font-size: 10px;"
                              onclick="document.getElementById('replaceInput<?= $item['id'] ?>_<?= $idx ?>').click()"
                              title="Replace image">
                ...
                <div class="form-text">Add new images or leave empty to keep current images (max 10 total). You cannot
                  remove the last existing image; replace it instead.</div>
              </div>
            </div>
