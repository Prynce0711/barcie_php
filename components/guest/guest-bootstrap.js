...
function showSection(sectionId, button, saveToStorage = true) {
  ...
  if (button) {
    ...
    const matchingButton = document.querySelector(
      `.sidebar-guest button[onclick*="${sectionId}"]`,
    );
    if (matchingButton) {
...
function filterItems() {
  const selectedType =
    document.querySelector('input[name="type"]:checked')?.value || "all";
    ...
    items.forEach((item, index) => {
      ...
      images = images.map((img) => {
        if (typeof img !== "string" || img.trim() === "")
          return "assets/images/imageBg/barcie_logo.jpg";
...
function ensureSectionsWork() {
  ...
  const visibleSections = document.querySelectorAll(
    '.content-section.active, .content-section[style*="block"]',
  );
  ...
  const navButtons = document.querySelectorAll(
    'button[onclick*="showSection"], .nav-link[data-section]',
  );
...
if (!window.guestPortalInitialized) {
  ...
  console.log(
    "Guest portal already initialized, skipping duplicate initialization",
  );
}
