    ...
    <div class="row">
      <div class="col-md-6">
        <div class="position-relative" style="cursor: pointer;" onclick="openGalleryFromModal(${JSON.stringify(images).replace(/"/g, "&quot;")}, 0, '${item.name.replace(/'/g, "\\'")}', ${item.id})">
          <img src="${images[0]}" class="img-fluid rounded mb-3" alt="${item.name}" style="max-height: 300px; width: 100%; object-fit: cover;" onerror="this.onerror=null;this.src=(window.BARCIE_LOGO_ALT_URL || window.BARCIE_LOGO_URL || 'public/images/imageBg/barcie_logo.jpg');">
          ${images.length > 1
        ? `
