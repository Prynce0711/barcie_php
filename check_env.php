...
if (class_exists('Dotenv\\Dotenv')) {
    try {
        // use a string-based callable so static analyzers don't require the Dotenv\Dotenv type to exist
        $createImmutable = ['Dotenv\\Dotenv', 'createImmutable'];
        if (is_callable($createImmutable)) {
            $dotenv = call_user_func($createImmutable, __DIR__);
            $dotenv->safeLoad();
        }
    } catch (\Exception $e) {
        // ignore
