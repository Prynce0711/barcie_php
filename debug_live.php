...
if (class_exists('Dotenv\\Dotenv')) {
    try {
        ...
        // Continue with runtime env if dotenv loading fails.
    }
}

// Optional key gate to reduce accidental exposure on live hosts.
$debugAccessKey = getenv('DEBUG_ACCESS_KEY') ?: ($_ENV['DEBUG_ACCESS_KEY'] ?? '');
if ($debugAccessKey !== '') {
    $providedKey = (string) ($_GET['key'] ?? '');
    if (!hash_equals((string) $debugAccessKey, $providedKey)) {
        http_response_code(403);
        echo 'Forbidden: invalid debug key.';
        exit;
    }
}
...
if (file_exists($dbConnectPath)) {
    ...
    $dbConnectionError = 'database/db_connect.php not found';
}
?>
<!DOCTYPE html>
<html>
...
<body>
    ...
    <div
        class="test <?php echo (isset($conn) && $conn instanceof mysqli && !$conn->connect_error) ? 'success' : 'error'; ?>">
        ...
        if (isset($conn) && $conn instanceof mysqli && !$conn->connect_error) {
            ...
            if ($dbResult) {
                ...
                $dbName = $dbRow[0] ?? 'Unknown';
            }

            echo '<p><span class="status ok">CONNECTED</span></p>';
            echo '<p><strong>Database:</strong> ' . htmlspecialchars($dbName) . '</p>';
        } else {
            $connectionMessage = $dbConnectionError;
            if (($connectionMessage === null || $connectionMessage === '') && isset($conn) && $conn instanceof mysqli && !empty($conn->connect_error)) {
                $connectionMessage = $conn->connect_error;
            }
            if ($connectionMessage === null || $connectionMessage === '') {
                $connectionMessage = 'Unknown error';
            }

            echo '<p><span class="status fail">FAILED</span></p>';
            echo '<p><strong>Error:</strong> ' . htmlspecialchars($connectionMessage) . '</p>';
        }
        ?>
    </div>
        ...
        async function testAPIs() {
            ...
            for (const endpoint of endpoints) {
                ...
                try {
                    ...
                    let preview = rawBody;
                    try {
                        preview = JSON.stringify(JSON.parse(rawBody));
                    } catch (parseError) {
                        // Keep raw response preview if response is not JSON.
                    }
                    ...
                    div.innerHTML = `
                        ...
                        <br><small>Response: ${escapeHtml(preview || '(empty body)')}</small>
                    `;
                } catch (error) {
                    div.innerHTML = `
                        <strong>${escapeHtml(endpoint.name)}:</strong> 
    ...
    ?>">
        ...
        if ($vendor_exists && file_exists($env_file)) {
            ...
            try {
                ...
                $smtp_user_masked = $smtp_user;
                if ($smtp_user !== 'NOT SET') {
                    if (strpos($smtp_user, '@') !== false) {
                        [$localPart, $domainPart] = explode('@', $smtp_user, 2);
                        if (strlen($localPart) > 2) {
                            $smtp_user_masked = substr($localPart, 0, 2) . str_repeat('*', max(strlen($localPart) - 2, 2)) . '@' . $domainPart;
                        } else {
                            $smtp_user_masked = str_repeat('*', strlen($localPart)) . '@' . $domainPart;
                        }
                    } elseif (strlen($smtp_user) > 4) {
                        $smtp_user_masked = substr($smtp_user, 0, 2) . '...' . substr($smtp_user, -2);
                    }
                }

                echo '<p><strong>SMTP Host:</strong> ' . htmlspecialchars($smtp_host) . '</p>';
                echo '<p><strong>SMTP Username:</strong> ' . htmlspecialchars($smtp_user_masked) . '</p>';
                ...
                try {
                    ...
                    if (preg_match('/\b(?:421|521)\b|lost connection|broken pipe/i', $lower)) {
    ...
    <script>
        ...
        if (emailTestForm) {
            ...
            emailTestForm.addEventListener('submit', async function (e) {
                e.preventDefault();
                ...
                try {
                    ...
                    try {
                        ...
                        // Keep data as null when response is not valid JSON.
                    }

                    if (response.ok && data && data.success) {
                        const message = escapeHtml(data.message || 'Email sent.');
                        resultDiv.innerHTML = `
                        ...
                        const fallback = rawBody ? rawBody.substring(0, 200) : 'Unknown error';
                        const message = escapeHtml((data && data.message) ? data.message : fallback);
                        resultDiv.innerHTML = `
