...
function updateReportData(data, reportType) {
    ...
    const sections = ['bookingReportSection', 'occupancyReportSection', 'revenueReportSection', 'guestReportSection', 'roomReportSection'];
...
function updateSummaryCards(summary) {
    ...
    if (document.getElementById('totalBookingsCount')) {
        document.getElementById('totalBookingsCount').textContent = formatNumber(summary.total_bookings || 0);
    }
    if (document.getElementById('totalRevenueAmount')) {
        document.getElementById('totalRevenueAmount').textContent = formatCurrency(summary.total_revenue || 0);
    }
    if (document.getElementById('occupancyRate')) {
        const rate = parseFloat(summary.occupancy_rate || 0);
        document.getElementById('occupancyRate').textContent = rate.toFixed(2) + '%';
    ...
    if (document.getElementById('totalGuestsCount')) {
        document.getElementById('totalGuestsCount').textContent = formatNumber(summary.total_guests || 0);
    }
}
...
function calculateOverviewSummary(data) {
    ...
    if (data.booking_reports && data.booking_reports.monthly_bookings) {
        totalBookings = data.booking_reports.monthly_bookings.reduce((sum, item) => sum + parseInt(item.count || 0), 0);
    }
...
function updateBookingReports(data) {
    ...
    if (statusTable && data.status_breakdown) {
        ...
        if (tbody) {
            tbody.innerHTML = '';
            const total = data.status_breakdown.reduce((sum, item) => sum + parseInt(item.count), 0);
...
function updateOccupancyReports(data) {
    ...
    if (document.getElementById('avgOccupancyRate')) {
        document.getElementById('avgOccupancyRate').textContent = (data.avg_occupancy_rate || 0) + '%';
    }
    
    // Peak occupancy date
    if (document.getElementById('peakOccupancyDate') && data.peak_occupancy) {
        document.getElementById('peakOccupancyDate').textContent = formatDate(data.peak_occupancy.date);
    }
    
    // Currently occupied
    if (document.getElementById('currentlyOccupied')) {
        document.getElementById('currentlyOccupied').textContent = data.currently_occupied || 0;
    }
    ...
    if (statusTable && data.room_status) {
        ...
        if (tbody) {
            tbody.innerHTML = '';
            const total = data.room_status.reduce((sum, item) => sum + parseInt(item.count), 0);
...
function updateRevenueReports(data) {
    ...
    if (document.getElementById('monthlyAvgRevenue')) {
        document.getElementById('monthlyAvgRevenue').textContent = formatCurrency(data.monthly_average || 0);
    }
    if (document.getElementById('dailyAvgRevenue')) {
        document.getElementById('dailyAvgRevenue').textContent = formatCurrency(data.daily_average || 0);
    }
...
function updateGuestReports(data) {
    ...
    if (document.getElementById('totalGuests')) {
        document.getElementById('totalGuests').textContent = formatNumber(data.total_guests || 0);
    }
    if (document.getElementById('avgStayLength')) {
        document.getElementById('avgStayLength').textContent = (data.avg_stay_length || 0) + ' days';
    }
    if (document.getElementById('returnGuests')) {
        document.getElementById('returnGuests').textContent = formatNumber(data.return_guests || 0);
    }
...
/**
 * Show toast notification (assumes toast-notification.js is loaded)
 */
