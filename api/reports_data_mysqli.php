...
function getOverviewData($conn, $startDate, $endDate, $roomType) {
    ...
    $sql = "SELECT COALESCE(SUM(b.total_price), 0) as total FROM bookings b
            ...
            AND b.status IN ('confirmed', 'checked_in', 'checked_out', 'approved')$roomTypeFilter";
    $stmt = $conn->prepare($sql);
    if (!empty($roomType)) {
        $stmt->bind_param('sss', $startDate, $endDate, $roomType);
    } else {
...
function getOccupancyReports($conn, $startDate, $endDate, $roomType) {
    ...
    $sql = "SELECT 
            CASE 
                WHEN b.id IS NOT NULL AND b.status IN ('confirmed', 'checked_in', 'approved') THEN 'Occupied'
                ELSE 'Available'
            END as status,
            COUNT(*) as count
            FROM items i
            ...
            WHERE i.item_type = 'room'" . ($roomType ? " AND i.type = ?" : "") . "
            GROUP BY status";
    $stmt = $conn->prepare($sql);
    if (!empty($roomType)) {
...
function getRevenueReports($conn, $startDate, $endDate, $roomType) {
    ...
    $sql = "SELECT DATE_FORMAT(b.checkin, '%Y-%m') as month,
            ...
            COALESCE(SUM(b.total_price), 0) as revenue, COUNT(*) as bookings
            FROM bookings b
            LEFT JOIN items i ON b.room_id = i.id
            WHERE b.checkin BETWEEN ? AND ?
            AND b.status IN ('confirmed', 'checked_in', 'checked_out', 'approved')$roomTypeFilter
...
function getGuestReports($conn, $startDate, $endDate, $roomType) {
    ...
    $sql = "SELECT COUNT(*) as return_guests FROM (
                ...
                LEFT JOIN items i ON b.room_id = i.id
                WHERE b.checkin BETWEEN ? AND ?$roomTypeFilter
                GROUP BY COALESCE(b.user_email, b.user_name)
                HAVING COUNT(*) > 1
            ) as returning";
    $stmt = $conn->prepare($sql);
    if (!empty($roomType)) {
    ...
    $sql = "SELECT COALESCE(b.user_name, 'Guest') as user_name,
            ...
            COUNT(*) as total_bookings, COALESCE(SUM(b.total_price), 0) as total_spent
            FROM bookings b
            LEFT JOIN items i ON b.room_id = i.id
            WHERE b.checkin BETWEEN ? AND ?$roomTypeFilter
            GROUP BY COALESCE(b.user_email, b.user_name), b.user_name
            ORDER BY total_bookings DESC, total_spent DESC LIMIT 10";
    $stmt = $conn->prepare($sql);
    if (!empty($roomType)) {
        $stmt->bind_param('sss', $startDate, $endDate, $roomType);
    } else {
...
function getRoomReports($conn, $startDate, $endDate, $roomType) {
    ...
    $sql = "SELECT i.name as room_number, i.type, COALESCE(COUNT(b.id), 0) as bookings,
            ...
            LEFT JOIN bookings b ON i.id = b.room_id 
                ...
                AND b.status IN ('confirmed', 'checked_in', 'checked_out', 'approved')
            WHERE i.item_type = 'room'" . ($roomType ? " AND i.type = ?" : "") . "
            GROUP BY i.id, i.name, i.type
    ...
    $sql = "SELECT i.type,
            ...
            SUM(CASE WHEN b.id IS NULL OR b.status NOT IN ('confirmed', 'checked_in', 'approved') THEN 1 ELSE 0 END) as available,
            SUM(CASE WHEN b.id IS NOT NULL AND b.status IN ('confirmed', 'checked_in', 'approved') THEN 1 ELSE 0 END) as occupied
            FROM items i
            LEFT JOIN bookings b ON i.id = b.room_id 
                AND CURDATE() BETWEEN b.checkin AND b.checkout
                AND b.status IN ('confirmed', 'checked_in', 'approved')
